...
; modified by h0ffman
;
; The intention of this replay is to provide an accurate
; ProTracker replayer with just a few issues fixed.
; Having used P61, LSP and the PT Replay by PHX, I've
; managed to find cases which those replayers don't
; support or sound weird. So.. I did this...
;
; Note that this is CIA only, although you could probably
; adapt it with a 3 stage copper interrupt.
;
;
; What's been fixed...
;
...
SHOW_RAS          = 1                                                            ; set to 1 to show raster time used
...
;
; CIA Driver 
; Stolen from Arnaud Carr� (aka Leonard / OXYGENE)
; modified by h0ffman for original PT Replayer
;
;*****************************************************************
...
.LSP_MainIrq:
                   ...
                   PUSHALL
                   bsr        mt_music
                   move.w     CIA_MusicBPM(pc),d0                                ; check if BMP changed in the middle of the music
                   cmp.w      CIA_CurrentBPM(pc),d0
                   beq.s      .noChg
                   lea        CIA_CurrentBPM(pc),a2			
                   move.w     d0,(a2)                                            ; current BPM
                   move.l     CIA_Clock(pc),d1
                   divu.w     d0,d1
                   move.b     d1,$bfd400
                   lsr.w      #8,d1
                   move.b     d1,$bfd500			
.noChg:            lea        .LSP_DmaconIrq(pc),a0
                   move.l     CIA_Vector(pc),a1
                   move.l     a0,(a1)
...
; CIA Stage 2 - enable dma and load volume
; you could easily shove a volume control in here.
...
.LSP_DmaconIrq:
                   ...
                   PUSHALL
                   bsr        mt_music2
                   move.l     CIA_Vector(pc),a0
                   pea        .LSP_PointerIrq(pc)
                   move.l     (a7)+,(a0)
                   move.b     #$19,$bfdf00                                       ; start timerB, one shot
                   ;POPM       d0/a0
                   POPALL
...
; CIA Stage 3 - load sample loop pointers
; this is also patched to instruments without notes, a very common chip music trick
...
mt_end             CLR.W      $DFF0A8
                ...
                ; 2nd stage cia, no dma wait fuckers!!

mt_music2
                   TST.B      mt_Enable
                   BEQ        .exit

                   MOVE.W     mt_DMACONtemp(PC),D0                               ; enable dma
                   OR.W       #$8000,D0
                   MOVE.W     D0,$DFF096
                   lea        $dff000,a0

                   move.w     mt_chan1temp+n_realvolume(pc),AUD0VOL(a0)
                   move.w     mt_chan2temp+n_realvolume(pc),AUD1VOL(a0)
                   move.w     mt_chan3temp+n_realvolume(pc),AUD2VOL(a0)
                   move.w     mt_chan4temp+n_realvolume(pc),AUD3VOL(a0)
.exit
                   rts

               ; 3rd stage cia, load loop pointers once dma is active
mt_music3
                   TST.B      mt_Enable
                   BEQ        .exit

                   MOVE.W     mt_DMACONtemp(PC),D0 
                   or.w       mt_LoadPointer(pc),d0 
                   LEA        $DFF000,A5
